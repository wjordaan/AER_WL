...
def input_UWI():
	...
	while True:
		try:
			...
			UWI = input("Please enter Alberta Well UWI: ")
			UWI = UWI.upper()
			UWI = re.sub("[/.\- A-Z ]",'',UWI)
			if len(UWI) != 15:
				raise ValueError
			...
			sys.stdout.write('Please enter valid 16 digit UWI ')
			print('\n')


	UWI_str= UWI[0:3]+'/'+UWI[3:5]+'-'+UWI[5:7]+'-'+UWI[7:10]+'-'+UWI[10:12]+'W'+UWI[12:13]+'/'+UWI[13:]

	print('Searching for UWI...')
						...
						for i,line in enumerate(searchlines):
							if UWI_str in line:
								...
								for l in searchlines[i:i+30]:
									x+=1
									lic_lines = searchlines[i-1:i+x]

								WL = re.search(r'(\d{7})',lic_lines[0])
								WL = WL.group()
